...
exec(`say -r 200 hola`, (err, stdout, stderr) => {
      if (err) {
...
function startWords(x){
  ...
  if(x==0){
    ...
    arrayWord1=_.shuffle(window.palabras);
  ...
  if(_f("queSay")==1){
     ...
     if(pasadaWord==arrayWord1.length-1){
        arrayWord=_.shuffle(window.palabras);
        pasadaWord=0;
...
var collision=0; //collision on off
...
//setTimeout(function(){ /*window.addEventListener('load', onloadHandler, false);*/ onloadHandler(); },5000) 
 ...
 //  scene.graph.push(blueLight);
  ...
  circles[i].decrement=0.08*vMax;
    ...
    circles[i].vz=rand(vMin,vMax); 
     ...
     <!--
      <a-sphere  dynamic-body="mass: 0" position="${circles[i].x+" "+circles[i].y+" "+circles[i].z}" material="transparent: true; opacity: 1.0;" radius="0.45" wireframe="false" id="sp-${i}"  src="#penguin-sledding" handler></a-sphere> -->
...
function aumenta(){
 ...
 // vMax=parseFloat(vMin).toFixed(5);
